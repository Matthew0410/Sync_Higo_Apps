<?php
	...
	function write_excel($phpexcel, $merchant_name, $arr_data)
	{
		...
		$phpexcel->getActiveSheet()->SetCellValue("H{$row}", 'Rate 3');
		$phpexcel->getActiveSheet()->SetCellValue("I{$row}", 'SP');
		$phpexcel->getActiveSheet()->SetCellValue("J{$row}", 'Rate 4');
		$phpexcel->getActiveSheet()->SetCellValue("K{$row}", 'Rate 5');
		$phpexcel->getActiveSheet()->SetCellValue("L{$row}", 'Rate 6');
		...
		foreach ($arr_data as $data)
		{
			...
			/*=========== Rate 3 ===========*/
			$phpexcel->getActiveSheet()->SetCellValue("H{$row}", "=IF(C{$row} > 0, F{$row} / C{$row}, 0)");
			$phpexcel->getActiveSheet()->getStyle("H{$row}")->getNumberFormat()->applyFromArray(array(
				'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
			));

			/*=========== SP ===========*/
			$phpexcel->getActiveSheet()->SetCellValue("I{$row}", $data->success);
			...
			/*=========== Rate 6 ===========*/
			$phpexcel->getActiveSheet()->SetCellValue("L{$row}", "=IF(C{$row} > 0, I{$row} / C{$row}, 0)");
