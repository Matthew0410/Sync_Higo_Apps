<?php
	...
	foreach ($arr_data as $day => $data)
	{
		...
		foreach ($data as $k => $v)
		{
			...
			$total_confirm += $count_tvc;
			$total_success += $count_sp;
	...
	$phpexcel->getActiveSheet()->SetCellValue("D{$row}", round($total_login / $count_date, 2));
	$phpexcel->getActiveSheet()->SetCellValue("E{$row}", round($total_data / $count_date, 2));
	$phpexcel->getActiveSheet()->SetCellValue("G{$row}", round($total_confirm / $count_date, 2));
	$phpexcel->getActiveSheet()->SetCellValue("J{$row}", round($total_success / $count_date, 2));
	...
	foreach ($arr_data as $day => $data)
	{
	  ...
	  foreach ($data as $k => $v)
	  {
	    ...
	    $data_M1 += ($v->mac != '' && $v->data > 1) ? 1 : 0	;
			$count_confirm += ($v->confirm != 0) ? 1 : 0;
			$count_success += ($v->success != 0) ? 1 : 0;
			...
			$phpexcel->getActiveSheet()->SetCellValue("Q{$row}", "=IF(AND(C{$row} <> \"\", C{$row} > 0), P{$row} / C{$row}, 0)");
			...
			$phpexcel->getActiveSheet()->SetCellValue("S{$row}", "=IF(AND(C{$row} <> \"\", C{$row} > 0), R{$row} / C{$row}, 0)");
	    ...
	    else
	    {
	      ...
	      $phpexcel->getActiveSheet()->getStyle("B{$row}:S{$row}")->applyFromArray(array(
			...
			if ($count_confirm != '') {
				$phpexcel->getActiveSheet()->SetCellValue("P{$first_data_row}", $count_confirm);
			}
			else {
				$phpexcel->getActiveSheet()->SetCellValue("P{$first_data_row}", 0);
			}
			if($count_success != ''){
				$phpexcel->getActiveSheet()->SetCellValue("R{$first_data_row}", $count_success);
			}
			else{
				$phpexcel->getActiveSheet()->SetCellValue("R{$first_data_row}", 0);
			}
	  }
	...
	// Summary of rate 7
	$phpexcel->getActiveSheet()->SetCellValue("G{$row}", "=IF(C{$last_row2} > 0, F{$last_row2}/C{$last_row2},0)");
	$phpexcel->getActiveSheet()->getStyle("G{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 8
	$phpexcel->getActiveSheet()->SetCellValue("I{$row}", "=IF(C{$last_row2} > 0, H{$last_row2}/C{$last_row2},0)");
	$phpexcel->getActiveSheet()->getStyle("I{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 9
	$phpexcel->getActiveSheet()->SetCellValue("K{$row}", "=IF(H{$last_row2} > 0, J{$last_row2}/H{$last_row2},0)");
	$phpexcel->getActiveSheet()->getStyle("K{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 10
	$phpexcel->getActiveSheet()->SetCellValue("M{$row}", "=IF(H{$last_row2} > 0, L{$last_row2}/H{$last_row2},0)");
	$phpexcel->getActiveSheet()->getStyle("M{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 12
	$phpexcel->getActiveSheet()->SetCellValue("Q{$row}", "=IF(C{$last_row2} > 0,P{$last_row2}/C{$last_row2},0)");
	$phpexcel->getActiveSheet()->getStyle("Q{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));

	// Summary of rate 13
	$phpexcel->getActiveSheet()->SetCellValue("S{$row}", "=IF(C{$last_row2} > 0, R{$last_row2}/C{$last_row2},0)");
	$phpexcel->getActiveSheet()->getStyle("S{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));
