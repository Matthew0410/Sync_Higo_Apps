<?php
  ...
  $arr_login_lookup = array();
	...
	$total_confirm = 0;
	$total_success = 0;
	$total_average_login = 0;
	$total_average_data = 0;
	$total_average_confirm = 0;
	...
	foreach ($arr_higo_router as $higo_router)
	{
		...
		$count_rate3 = 0;
		$count_rate4 = 0;
		$count_average_login = 0;
		$count_average_data = 0;
		$count_average_confirm = 0;
		$count_average_success = 0;
      ...
      foreach ($arr_mac_lookup[$higo_router->id][date('Y-m-d', $date)] as $mac)
		...
		$count_average_login = ($count_login > 0) ? round($count_login / $count_date, 2) : 0;
		$count_average_data = ($count_data > 0) ? round($count_data / $count_date, 2) : 0;
		$count_average_confirm = ($count_confirm > 0) ? round($count_confirm / $count_date, 2) : 0;
		$count_average_success = ($count_success > 0) ? round($count_success / $count_date, 2) : 0;

		$total_average_login += $count_average_login;
		$total_average_data += $count_average_data;
		$total_average_confirm += $count_average_confirm;
		$total_average_success += $count_average_success;
		...
		/*========= Rate 1 ========= */
		$phpexcel->getActiveSheet()->SetCellValue("H{$row}", "=IF(F{$row}>0,G{$row}/F{$row},0)");
		...
		/*========= Rate 3 ========= */
		$phpexcel->getActiveSheet()->SetCellValue("K{$row}", "=IF(F{$row}>0,I{$row}/F{$row},0)");
		$phpexcel->getActiveSheet()->getStyle("K{$row}")->getNumberFormat()->applyFromArray(array(
			'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		));

		/*========= Rate 4 ========= */
		$phpexcel->getActiveSheet()->SetCellValue("M{$row}", "=IF(I{$row}>0,L{$row}/I{$row},0)");
		$phpexcel->getActiveSheet()->getStyle("M{$row}")->getNumberFormat()->applyFromArray(array(
			'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		));

		/*========= Rate 5 ========= */
		$phpexcel->getActiveSheet()->SetCellValue("N{$row}", "=IF(G{$row}>0,L{$row}/G{$row},0)");
		$phpexcel->getActiveSheet()->getStyle("N{$row}")->getNumberFormat()->applyFromArray(array(
			'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		));

		/*========= Rate 6 ========= */
		$phpexcel->getActiveSheet()->SetCellValue("O{$row}", "=IF(F{$row}>0,L{$row}/F{$row},0)");
		...
		$phpexcel->getActiveSheet()->SetCellValue("F{$row}", $count_average_login);

		$phpexcel->getActiveSheet()->SetCellValue("G{$row}", $count_average_data);

		$phpexcel->getActiveSheet()->SetCellValue("I{$row}", $count_average_confirm);

		$phpexcel->getActiveSheet()->SetCellValue("L{$row}", $count_average_success);

		/*========== AVERAGE 2 ==========*/
		$row += 1;

		$phpexcel->getActiveSheet()->SetCellValue("F{$row}", round($count_login / $count_date2, 2));
		$phpexcel->getActiveSheet()->SetCellValue("G{$row}", round($count_data / $count_date2, 2));
		$phpexcel->getActiveSheet()->SetCellValue("I{$row}", round($count_confirm / $count_date2, 2));
		$phpexcel->getActiveSheet()->SetCellValue("L{$row}", round($count_success / $count_date2, 2));
	...
	$phpexcel->getActiveSheet()->SetCellValue("F{$row}", $total_average_login);
	$phpexcel->getActiveSheet()->SetCellValue("G{$row}", $total_average_data);
	$phpexcel->getActiveSheet()->SetCellValue("I{$row}", $total_average_confirm);
	$phpexcel->getActiveSheet()->SetCellValue("L{$row}", $total_average_success);
	...
	$total_M1Mac = 0;
	$total_count_M0Mac = 0;
	$total_count_confirm = 0;
	$total_count_success = 0;
	$total_average_login2 = 0;
	$total_average_M0Mac = 0;
	$total_average_1Mac = 0;
	$total_average_M1Mac = 0;
	$total_average_tvc = 0;
	$total_average_sp = 0;
	...
	foreach ($arr_higo_router as $higo_router)
	{
	  ...
	  {
			$count_date += (!isset($arr_login_lookup[$higo_router->id][date('Y-m-d', $date)])) ? 1 : 0;
			// $count_date2 += (isset($arr_login_lookup[$higo_router->id][date('Y-m-d', $date)])) ? 1 : 0;
			$count_date2 +=1;
	    ...
	    foreach ($arr_mac_lookup[$higo_router->id][date('Y-m-d', $date)] as $mac => $v)
	    {
				...
				$count_login += (isset($arr_login_lookup[$higo_router->id][date('Y-m-d', $date)][$mac])) ? 1 : 0;
				$data0Mac += (!isset($arr_log_lookup[$higo_router->id][date('Y-m-d', $date)][$mac])) ? 1 : 0;
				$data = (isset($arr_log_lookup[$higo_router->id][date('Y-m-d', $date)][$mac])) ? $arr_log_lookup[$higo_router->id][date('Y-m-d', $date)][$mac]->count_data : 0;
				$confirm = (isset($arr_confirm_lookup[$higo_router->id][date('Y-m-d', $date)][$mac])) ? $arr_confirm_lookup[$higo_router->id][date('Y-m-d', $date)][$mac] : 0;
        $success = (isset($arr_alogin_lookup[$higo_router->id][date('Y-m-d', $date)][$mac])) ? $arr_alogin_lookup[$higo_router->id][date('Y-m-d', $date)][$mac] : 0;

				$confirm2 += ($confirm != 0) ? 1 : 0;
				$success2 += ($success != 0) ? 1 : 0;
				...
				$range_rate78 = ($count_login != 0) ? 1 : 0;
				$range_rate91 = ($dataM0Mac != 0 || $data1Mac != 0) ? 1 : 0;
				$range_tvc = ($confirm2 != 0) ? 1 : 0;
				$range_sp = ($success2 != 0) ? 1 : 0;
	    }
			$count_date_rate78 += $range_rate78;
			$count_date_rate91 += $range_rate91;
			$count_date_1Mac += $range_1Mac;
			$count_date_M0Mac += $range_M0Mac;
			$count_date_M1Mac += $range_M1Mac;
			$count_date_tvc += $range_tvc;
			$count_date_sp += $range_sp;
			...
			$count_confirm2 += $confirm2;
			$count_success2 += $success2;

			$total_count_M0Mac += $dataM0Mac;
			$total_1Mac += $data1Mac;
			$total_M1Mac += $dataM1Mac;
			$total_count_confirm += $confirm2;
			$total_count_success += $success2;

			// echo date('Y-m-d', $date). '========'. $count_login. '<br/>';

			$count_rate7 += ($count_login != 0) ? round(($data0Mac / $count_login) * 100) : 0;
			$count_rate8 += ($count_login != 0) ? round(($dataM0Mac / $count_login) * 100) : 0;
			$count_rate9 += ($dataM0Mac != 0 || $data1Mac != 0) ? round(($data1Mac / $dataM0Mac) * 100) : 0;
			$count_rate10 += ($dataM0Mac != 0 || $dataM1Mac != 0) ? round(($dataM1Mac / $dataM0Mac) * 100) : 0;
			$count_rate12 += ($count_login != '' && $count_login > 0) ? round(($confirm2 / $count_login) * 100) : 0;
			$count_rate13 += ($count_login != '' && $count_login > 0) ? round(($success2 / $count_login) * 100) : 0;
			$count_0Mac += $data0Mac;
		...
		$averageLogin += ($count_hMac != '') ? round($total_count_login / $count_hMac, 2) : 0;
		$averageRate7 += ($count_date_rate78 != '') ? round($count_rate7 / $count_date_rate78) / 100 : 0;
		$averageRate8 += ($count_date_rate78 != '') ? round($count_rate8 / $count_date_rate78) / 100 : 0;
		$averageRate9 += ($count_date_rate91 != '') ? round($count_rate9 / $count_date_rate91) / 100 : 0;
		$averageRate10 += ($count_date_rate91 != '') ? round($count_rate10 / $count_date_rate91) / 100 : 0;
		$averageRate12 += ($count_date2 != '') ? round($count_rate12 / $count_date2) / 100 : 0;
		$averageRate13 += ($count_date2 != '') ? round($count_rate13 / $count_date2) / 100 : 0;

		$averageM0Mac += ($count_date_M0Mac != '') ? round($count_M0Mac / $count_date_M0Mac, 2) : 0;
		$average1Mac += ($count_date_1Mac != '') ? round($count_1Mac / $count_date_1Mac, 2) : 0;
		$averageM1Mac += ($count_date_M1Mac != '') ? round($count_M1Mac / $count_date_M1Mac, 2) : 0;
		$averageTvc += ($count_date_tvc != '') ? round($count_confirm2 / $count_date_tvc, 2) : 0;
		$averageSp += ($count_date_sp != '') ? round($count_success2/ $count_date_sp, 2) : 0;

		$total_average_login2 += $averageLogin;
		$total_average_M0Mac += $averageM0Mac;
		$total_average_1Mac += $average1Mac;
		$total_average_M1Mac += $averageM1Mac;
		$total_average_tvc += $averageTvc;
		$total_average_sp += $averageSp;
	  ...
	  $phpexcel->getActiveSheet()->SetCellValue("F{$row}", $total_count_login);
		...
		$phpexcel->getActiveSheet()->SetCellValue("S{$row}", $count_confirm2);
		$phpexcel->getActiveSheet()->SetCellValue("T{$row}", "=IF(F{$row} > 0, S{$row}/F{$row},0)");
		$phpexcel->getActiveSheet()->getStyle("T{$row}")->getNumberFormat()->applyFromArray(array(
			'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		));
		$phpexcel->getActiveSheet()->SetCellValue("U{$row}", $count_success2);
		$phpexcel->getActiveSheet()->SetCellValue("V{$row}", "=IF(F{$row} > 0, U{$row}/F{$row},0)");
		$phpexcel->getActiveSheet()->getStyle("V{$row}")->getNumberFormat()->applyFromArray(array(
			'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		));

	  $row += 1;

		$phpexcel->getActiveSheet()->SetCellValue("F{$row}", $averageLogin);
		$phpexcel->getActiveSheet()->getStyle("F{$row}")->getNumberFormat()->setFormatCode('0.00');

		if ($count_login != '') {
			$phpexcel->getActiveSheet()->SetCellValue("J{$row}", $averageRate7);
			$phpexcel->getActiveSheet()->getStyle("J{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
			$phpexcel->getActiveSheet()->SetCellValue("L{$row}", $averageRate8);
			$phpexcel->getActiveSheet()->getStyle("L{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
			$phpexcel->getActiveSheet()->SetCellValue("N{$row}", $averageRate9);
			$phpexcel->getActiveSheet()->getStyle("N{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
			$phpexcel->getActiveSheet()->SetCellValue("P{$row}", $averageRate10);
			$phpexcel->getActiveSheet()->getStyle("P{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
			$phpexcel->getActiveSheet()->SetCellValue("T{$row}", $averageRate12);
			$phpexcel->getActiveSheet()->getStyle("T{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
			$phpexcel->getActiveSheet()->SetCellValue("V{$row}", $averageRate13);
			$phpexcel->getActiveSheet()->getStyle("V{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
		}
			...
			$phpexcel->getActiveSheet()->SetCellValue("T{$row}", 0);
			$phpexcel->getActiveSheet()->getStyle("T{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
			$phpexcel->getActiveSheet()->SetCellValue("V{$row}", 0);
			$phpexcel->getActiveSheet()->getStyle("V{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
		}
		...
		if ($count_login != '') {
			$phpexcel->getActiveSheet()->SetCellValue("K{$row}", $averageM0Mac);
		}
	  ...
	  // ));
		...
		if($count_login != '') {
			$phpexcel->getActiveSheet()->SetCellValue("M{$row}", $average1Mac);
		}
		else {
		...
		if($count_login != '') {
			$phpexcel->getActiveSheet()->SetCellValue("O{$row}", $averageM1Mac);
		}
		else {
			$phpexcel->getActiveSheet()->SetCellValue("O{$row}", 0);
		}
		if($count_date_tvc != ''){
			$phpexcel->getActiveSheet()->SetCellValue("S{$row}", $averageTvc);
		}
		else {
			$phpexcel->getActiveSheet()->SetCellValue("S{$row}", 0);
		}
		if($count_date_sp != ''){
			$phpexcel->getActiveSheet()->SetCellValue("U{$row}", $averageSp);
		}
		else {
			$phpexcel->getActiveSheet()->SetCellValue("U{$row}", 0);
		}
	...
	$phpexcel->getActiveSheet()->SetCellValue("S{$row}", $total_count_confirm);
	$phpexcel->getActiveSheet()->SetCellValue("T{$row}", "=IF(F{$row}>0, S{$row}/F{$row}, 0)");
	$phpexcel->getActiveSheet()->getStyle("T{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));
	$phpexcel->getActiveSheet()->SetCellValue("U{$row}", $total_count_success);
	$phpexcel->getActiveSheet()->SetCellValue("V{$row}", "=IF(F{$row}>0, U{$row}/F{$row}, 0)");
	$phpexcel->getActiveSheet()->getStyle("V{$row}")->getNumberFormat()->applyFromArray(array(
		'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
	));
	...
	$phpexcel->getActiveSheet()->SetCellValue("F{$row}", $total_average_login2);

	$phpexcel->getActiveSheet()->SetCellValue("K{$row}", $total_average_M0Mac);

	$phpexcel->getActiveSheet()->SetCellValue("M{$row}", $total_average_1Mac);

	$phpexcel->getActiveSheet()->SetCellValue("O{$row}", $total_average_M1Mac);

	$phpexcel->getActiveSheet()->SetCellValue("S{$row}", $total_average_tvc);

	$phpexcel->getActiveSheet()->SetCellValue("U{$row}", $total_average_sp);
